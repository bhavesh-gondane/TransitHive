...
function UserRegister() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post('http://localhost:5205/api/auth/user/register', formData);
      console.log("inuser register",response.data.message);
      if (response.data.message ==="Email already in use."){
        setApiError(response.data.message);
        return;}
      setRegistrationMessage(response.data.message); // Set the success message from the backend
      setFormData({ name: '', email: '', password: '', phone: '', city: '' }); // Clear the form
      navigate('/login/user',{
        state: { message: 'Registration successful! Please Check Your Mail to Verify Your Account' }
      });
