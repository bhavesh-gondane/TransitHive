...
const SuspendedVendors = () => {
  ...
  useEffect(() => {
    const fetchSuspendedVendors = async () => {
      try {
        const response = await axios.get('http://localhost:5205/api/Vendor/status/SUSPENDED');
  ...
  const handleReactivate = async (vendorId) => {
    try {
      const response = await axios.patch(`http://localhost:5205/api/Vendor/${vendorId}/status?status=APPROVED&comment=APPROVED`);
